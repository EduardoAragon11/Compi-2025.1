...
#include "exp.h"
#include "parser.h"
#include "../../s7-Lab10/exp.h"
#include "../../s8-lab12/exp.h"
...
Stm* Parser::parseStatement() {
    ...
    if (match(Token::ID)) {
        ...
        e = parseCExp();
        if(!match(Token::DO)){
            cout<<"se esperaba un 'do' despues de la expresion"<<endl;
            exit(1);
        }
        tb = parseBody();
        if(!match(Token::ENDWHILE)){
            cout<<"se esperaba un 'endwhile' despues de la declaracion"<<endl;
            exit(1);
        }
        ...
        if(!match(Token::ID)){
            cout << "Error: se esperaba 'id' después de '('." << endl;
            exit(1);
        }
        string id1 = previous->text;
        if(!match(Token::ASSIGN)){
            cout << "Error: se esperaba '=' después de 'id'." << endl;
            exit(1);
        }
        Exp* start = parseCExp();
        AssignStatement* ini = new AssignStatement(id1, start);
        if (!match(Token::COMA)) {
        ...
        Exp* step = parseCExp();
        AssignStatement* end = new AssignStatement(id2, step);
        if (!match(Token::PD)) {
            cout << "Error: se esperaba ')' después de la expresión." << endl;
            exit(1);
        }
        ...
        s = new ForStatement(ini, cond,end,tb);
        }
